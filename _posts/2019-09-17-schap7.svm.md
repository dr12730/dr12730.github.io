...
$$w^{*} = \sum\limits_{i=1}^{N} \alpha_i^{*} y_i x_i \tag{2.6}\label{2.7}$$
...
$$\alpha_1^{new} = \alpha_1^{old} + y_1y_2 (\alpha_2^{old} - \alpha_2^{new}) \tag{3.5}$$


> 其中：
> - $y_1 = y_2$：
>
>   $$L = \max(0, \alpha_2^{old} - \alpha_1^{old}), \quad H = \min(C, C + \alpha_2^{odl} - \alpha_1^{old})$$
> 
> - $y_1 \neq y_2$：
>
...
- 输入：训练数据 $D = \left\\{ (x_1, y_1), (x_2, y_2), \dots, (x_N, y_N) \right\\}$
...
- 过程：
    ...
    3. 若在精度 $\sigma$ 范围内满足停机条件则转(4)；否则令 $k = k+1$ ，转(2)
        ...
        - $y_i \cdot g(x_i) = \begin{cases} \ge 1, & \left\\{ x_i \mid \alpha_i = 0 \right\\} \\ = 1, & \left\\{ x_i \mid 0 < \alpha_i < C \right\\} \\ le 1, & \left\\{ x_i \mid \alpha_i = C \right\\} \end{cases}$，其中： $g(x_i) = \sum\limits_{j=1}^{N} a_j y_j K(x_j, x_i) + b$ 
