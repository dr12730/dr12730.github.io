...
$$\mathop{\arg\min}\limits_{w, b, \xi} \left[\dfrac{1}{2} \mid\mid w \mid\mid ^2 + C \sum\limits_{i=1}^{N} \xi_i \right] \tag{2.1}$$

s.t：

$$ y_i(w^T \cdot x_i + b) + \xi_i \ge 1 \tag{2.2}$$

$$\xi_i \ge 0 \tag{2.3}$$

> 最小化目标函数 $(2.1)$ 包含两层含义：使 $\dfrac{1}{2} \mid\mid w \mid\mid ^2$ 尽可能小即间隔尽可能大，同时让落在分隔区的特异样本尽可能少（误分类点尽可能少），C 调和两者的系数

通过学习算法即软间隔最大化，可以找到 $(w^{*}, b^{*})$ ，从而得到分离超平面和分类决策函数。分离超平面和相应的分类决策函数一起，就成为线性支持向量机

### 2.1 学习算法的对偶算法

原始问题  $(2.1), (2.2), (2.3)$ 的对偶问题如下：
...
$$w^{*} = \sum\limits_{i=1}^{N} \alpha_i^{*} y_i x_i \tag{2.6}$$

$$b^{*} = y_j - \sum\limits_{i=1}^{N} y_i \alpha_i^{*} (x_i \cdot x_j) \tag{2.7}$$
...
1. 选择惩罚参数 C > 0，构造并求解凸二次优化问题： $(2.3) ~ (2.4)$

    求得最优解 $\alpha^{*} = (\alpha_1^{*}, \alpha_2^{*}, \dots, \alpha_N^{*})^T$

2. 计算 $(2.6)$

    选择 $\alpha^{*}$ 的一个分量 $\alpha^{*}_j$ 计算 $(2.7)$ 
