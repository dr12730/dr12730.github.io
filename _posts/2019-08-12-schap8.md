...
- 流程
  ...
  $$\alpha_m, G_m = arg \min\limits_{\alpha, G}\sum\limits_{i=1}^N \bar{w}_{mi} \exp [-y_i \alpha G(x_i)] &= \sum\limits_{i \in M1} \bar{w}_{mi} e^{-\alpha} + \sum\limits_{i \in M2} \bar{w}_{mi} e^{\alpha}\end{split} &= \sum\limits_{i \in M1} \bar{w}_{mi} e^{-\alpha} + \sum\limits_{i \in M2} \bar{w}_{mi} e^{-\alpha} + \sum\limits_{i \in M2} \bar{w}_{mi} \left(e^{\alpha} - e^{-\alpha} \right) &= \sum\limits_{i=1}{M} e^{-\alpha} \bar{w}_{mi} \left(e^{\alpha} - e^{-\alpha} \right)+ \end{split} \sum\limits_{i \in M2} \bar{w}_{mi} I(y_i \neq G(x_i)) $$

  先求 $G^*_m(x) = arg\min\limits_{G} \sum\limits_{i} \bar{w}_{mi} I(y_i \neq G(x_i))$
  在求：

  $$
  \alpha_m = arg \min\limits_{\alpha} \sum\limits_{i} \bar{w}_{mi} \exp (\alpha y_i G^*(x_i)) &= arg \min\limits_{\alpha} \sum\limits_{i \in M1}\bar{w}_{mi} e^{-\alpha} + \sum\limits_{i \in M2} \bar{w}_{mi} e^{\alpha} &= \left( e^{\alpha} - e^{-\alpha} \right) \sum\limits_{i} \bar{w}_{mi} I(y_i \neq G(x_i)) + e^{-\alpha} \sum\limits_{i}\bar{w}_{mi}
  \end{split}$$
  $$

  求导：

  $$ \frac{\partial L}{\partial \alpha_m} = \left( e^{\alpha} - e^{-\alpha} \right) \sum\limits_{i} \bar{w}_{mi} I - \sum\limits_{i}  e^{-\alpha} \bar{w}_{mi} = 0$$

  $$
  \alpha &= \frac{1}{2} \ln \frac{\sum\limits_{i} \bar{w}_{mi} - \sum\limits_{i} \bar{w}_{mi} I}{\sum\limits_{i}\bar{w}_{mi}I}
  ...
  $$ \bar{w}_{mi} = \exp \left( y_i f_{m-1}(x_i)\right) = \exp (-y_i \sum\limits_{j=1}^{M-1}\alpha_j G_j(x_i)) = \prod_j \exp(-y_i\alpha_j G_j(x_i))$$
