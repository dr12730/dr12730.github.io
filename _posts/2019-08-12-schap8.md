...
- 流程
  ...
  2. 最小化损失函数求 $G^*_m(x)$ 和 $\alpha_m$
     ...
     $$\begin{split} 
     \alpha_m, G_m &= arg \min\limits_{\alpha, G}\sum\limits_{i=1}^N \bar{w}_{mi} \exp [-y_i \alpha G(x_i)] \
     &= \sum\limits_{i \in M1} \bar{w}_{mi} e^{-\alpha} + \sum\limits_{i \in M2} \bar{w}_{mi} e^{\alpha} \
     ...
     &= \sum\limits_{i=1}{M} e^{-\alpha} \bar{w}_{mi} \left(e^{\alpha} - e^{-\alpha} \right)+ \end{split} \sum\limits_{i \in M2} \bar{w}_{mi} I(y_i \neq G(x_i)) \end{split}$$
     ...
     $$\begin{split} \alpha_m &= arg \min\limits_{\alpha} \sum\limits_{i} \bar{w}_{mi} \exp (\alpha y_i G^*(x_i))\ &= arg \min\limits_{\alpha} \sum\limits_{i \in M1}\bar{w}_{mi} e^{-\alpha} + \sum\limits_{i \in M2} \bar{w}_{mi} e^{\alpha} \ &= \left( e^{\alpha} - e^{-\alpha} \right) \sum\limits_{i} \bar{w}_{mi} I(y_i \neq G(x_i)) + e^{-\alpha} \sum\limits_{i}\bar{w}_{mi}\end{split}$$
