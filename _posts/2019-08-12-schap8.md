...
- 流程
  ...
  2. 对 $m = 1, 2, ...., M$
     ...
     - 用权值分布 $D_m$ 的训练数据集学习，得到基本分类器 $G_m(x): X \to \left{-1, +1\right}$
...
- 流程
  ...
  2. 最小化损失函数求 $G^*_m(x)$ 和 $\alpha_m$
     ...
     $$\begin{split}\dfrac{\partial L}{\partial \alpha_m} &= \left( e^\alpha + e^{-\alpha} \right) \sum \bar{w}_i I - \sum e^{-\alpha} \bar{w}_i \\ &= \left( e^{2\alpha} +1 \right) \sum \bar{w}_i I - \sum\bar{w}_i = 0\end{split}$$
